...
def parse_args():
    ...
    parser.add_argument('--data_dir', nargs='?', default='LHCE/')
    ...
    parser.add_argument('--round_idx', default=2, type=int)

    parser.add_argument('--seed', default=42, type=int, help='Seed for random number generators')
    parser.add_argument('--budget', default=2, type=int, help='Time budget')
    parser.add_argument('--bsize', default=50, type=int, help='batch size')

    parser.add_argument('--gamma', default=1.0, type=float,help='kernel bandwidth for Gaussian kernel')
    parser.add_argument('--init_beta', default=5.0*0.5**2, type=float, help='initial exploration constant in Thm 1')
    parser.add_argument('--epsilon', default=0.5, type=float, help='decay constant for exploration')
    # 根据新数据系列的基线测试数据更改了部分超参数
    # Some hyperparameters are changed based on baseline test data from new data series
    parser.add_argument('--likelihood_std', default=0.89, type=float, help='standard deviation for the likelihood std')
    parser.add_argument('--standardization_mean', default=54.0, type=float, help='mean lifetime from batch')
    parser.add_argument('--standardization_std', default=1.12, type=float, help='std lifetime from batch')
...
class BayesGap(object):
    ...
    def __init__(self, args):
        ...
        self.prev_early_pred_file = os.path.join(args.data_dir, args.early_pred_dir, str(args.round_idx) + '.csv') # note this is for previous round
        self.arm_bounds_file = os.path.join(args.data_dir, args.arm_bounds_dir, str(args.round_idx) + '.pkl')
        self.next_batch_file = os.path.join(args.data_dir, args.next_batch_dir, str(args.round_idx+1) + '.csv')
    ...
    def run(self):
        ...
        # 获取上一轮次的 PC 协议和对应的结果
        # Get the PC protocol and the corresponding result of the previous round
        ...
        # 获取已经执行过的协议的高斯核
        # Get the Gaussian kernel of the protocols that have been executed
        ...
        # 获取已经执行过的协议的标准结果
        # Get the standardized results of the protocols that have been executed
        data = early_pred[:, -1]
        ...
        for index,arm in enumerate(batch_arms):
            Tools.XML.XMLfile(arm, round_idx=6, stop_volt1=4.3, stop_volt2=2.8).to_xml(index, LOOP=100)
    ...
    def get_proposal_solution(self, proposal_gaps, proposal_arms, upper_bounds, lower_bounds):
        if proposal_arms == []:
            best_arm = np.argmax(upper_bounds)
            best_arm_params = self.param_space[best_arm]
            proposal_gaps.append(np.amax(upper_bounds) - lower_bounds[best_arm])

        else:
            J_prev_round = proposal_arms[self.round_idx-2]
...
def main():
    ...
    if args.round_idx != 0:
        print('Best arm until round', args.round_idx, 'is', best_arm_params)
